 ...
 * @return bool
 */
if ( ! function_exists( 'vedanta_is_login_page' ) ) {
	function vedanta_is_login_page() {
		return in_array( $GLOBALS['pagenow'], array( 'wp-login.php', 'wp-register.php' ) );
	}
}

//check if site is in undermaintatance
add_action( 'init', 'vedanta_under_maintenance', 21 );
function vedanta_under_maintenance(){
    global $ved_options;
	
	if( is_admin() || vedanta_is_login_page() || ( defined('XMLRPC_REQUEST') && XMLRPC_REQUEST ) ){
		return;
	}
	
	do_action( 'vedanta_maintenance_before' );
	
    $enable_maintenance = isset( $ved_options['ved_enable_maintenance'] ) ? $ved_options['ved_enable_maintenance'] : 0;
	
    if( $enable_maintenance ){
		add_filter( 'body_class', 'vedanta_maintenance_body_class' );
...
function vedanta_maintenance_body_class( $classes ) {
	global $ved_options;
	
	if( is_admin() || vedanta_is_login_page() ){
		return $classes;
	}
