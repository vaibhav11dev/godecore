...
class sidebar_generator {
		...
		if ( is_array( $sidebars ) ) {
			foreach ( $sidebars as $sidebar ) {
				...
				register_sidebar( array(
				    'name' => $sidebar,
				    'id' => 'vedanta-custom-sidebar-' . strtolower( $sidebar_class ),
			...
			if ( is_array( $sidebars ) && !empty( $sidebars ) ) {
				...
				foreach ( $sidebars as $sidebar ) {
					...
					<tr class="<?php echo esc_attr($alt); ?>">
					    ...
					    <td><a href="javascript:void(0);" onclick="return remove_sidebar_link('<?php echo esc_js($sidebar); ?>',<?php echo (int)$cnt + 1; ?>);" title="Remove this sidebar">remove</a></td>
					</tr>
					<?php
					$cnt++;
				}
	...
	public static function edit_form() {
		...
		<p><?php esc_html_e( 'Please select the sidebar you would like to display on this page. Note: You must first create the sidebar under Appearance > Sidebars.', 'godecore' ); ?>
		</p>
		<ul>
		    <?php
