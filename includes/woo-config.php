 ...
 * Vedanta Framework
 *
 * WARNING: This file is part of the Vedanta Core Framework.
 ...
 */
function vedanta_woocommerce_header_add_to_cart_fragment1( $fragments ) {
	global $woocommerce;
	...
	<?php
	$fragments['.header-ajax-cart .mini-item-counter'] = ob_get_clean();

	return $fragments;
}

add_filter( 'woocommerce_add_to_cart_fragments', 'vedanta_woocommerce_header_add_to_cart_fragment1', 9 );

function vedanta_woocommerce_header_add_to_cart_fragment2( $fragments ) {
	global $woocommerce;
	ob_start();
...
add_filter( 'woocommerce_add_to_cart_fragments', 'vedanta_woocommerce_header_add_to_cart_fragment2', 10 );

// Remove product in the cart using ajax
function vedanta_ajax_product_remove() {
	// Get mini cart
	ob_start();
...
add_action( 'wp_ajax_product_remove', 'vedanta_ajax_product_remove' );
add_action( 'wp_ajax_nopriv_product_remove', 'vedanta_ajax_product_remove' );
 ...
 *
 * */
function vedanta_woocommerce_price_html( $price, $product ) {
	return preg_replace( '@(<del>.*?</del>).*?(<ins>.*?</ins>)@misx', '$2 $1', $price );
}

add_filter( 'woocommerce_get_price_html', 'vedanta_woocommerce_price_html', 100, 2 );
 ...
 * @return $args
 */
function vedanta_related_products_args( $args ) {
	$args[ 'posts_per_page' ] = 5; // number of related products
	return $args;
}

add_filter( 'woocommerce_output_related_products_args', 'vedanta_related_products_args', 20 );
 ...
 * @return boolean
 */
function vedanta_shop_title() {
	return false;
}

add_filter( 'woocommerce_show_page_title', 'vedanta_shop_title', 10 );
...
//add_action( 'woocommerce_shop_loop_item_title', 'vedanta_woocommerce_template_loop_product_title', 10 );
 ...
 * 
 */
function vedanta_woocommerce_ordering() {
	$ved_woocommerce_ordering = vedanta_get_option( 'ved_woocommerce_ordering', '0' );

	// remove default shorting option
	remove_action( 'woocommerce_before_shop_loop', 'woocommerce_result_count', 20 );
	remove_action( 'woocommerce_before_shop_loop', 'woocommerce_catalog_ordering', 30 );
	if ( !$ved_woocommerce_ordering ) {
		add_action( 'woocommerce_before_shop_loop', 'vedanta_woocommerce_catalog_ordering', 30 );
		add_action( 'woocommerce_get_catalog_ordering_args', 'vedanta_woocommerce_get_catalog_ordering_args', 20 );
		add_filter( 'loop_shop_per_page', 'vedanta_loop_shop_per_page' );
	}
}

add_action( 'init', 'vedanta_woocommerce_ordering' );

function vedanta_woocommerce_catalog_ordering() {
	global $wp_query;
	$total = $wp_query->found_posts;

	$ved_woo_items	 = vedanta_get_option( 'ved_woo_items', '12' );
	...
	$html	 .= '<a href="#" class="grid-view ved-shop-view active pull-left" data-view="grid"></a>';
	...
	$html	 .= '<span data-toggle="dropdown" class="current-li"><span class="current-li-content"><a>' . esc_html__( 'Sort by', 'godecore' ) . ' <strong>' . esc_html__( 'Default Order', 'godecore' ) . '</strong></a><i class="fa fa-angle-down"></i></span></span>';
	$html	 .= '<ul class="dropdown-menu">';
	$html	 .= '<li class="' . (($pob == 'default') ? 'current' : '') . '"><a href="' . esc_url( vedanta_addURLParameter( $query_string, 'product_orderby', 'default' ) ) . '">' . esc_html__( 'Sort by', 'godecore' ) . ' <strong>' . esc_html__( 'Default Order', 'godecore' ) . '</strong></a></li>';
	$html	 .= '<li class="' . (($pob == 'name') ? 'current' : '') . '"><a href="' . esc_url( vedanta_addURLParameter( $query_string, 'product_orderby', 'name' ) ) . '">' . esc_html__( 'Sort by', 'godecore' ) . ' <strong>' . esc_html__( 'Name', 'godecore' ) . '</strong></a></li>';
	$html	 .= '<li class="' . (($pob == 'price') ? 'current' : '') . '"><a href="' . esc_url( vedanta_addURLParameter( $query_string, 'product_orderby', 'price' ) ) . '">' . esc_html__( 'Sort by', 'godecore' ) . ' <strong>' . esc_html__( 'Price', 'godecore' ) . '</strong></a></li>';
	$html	 .= '<li class="' . (($pob == 'date') ? 'current' : '') . '"><a href="' . esc_url( vedanta_addURLParameter( $query_string, 'product_orderby', 'date' ) ) . '">' . esc_html__( 'Sort by', 'godecore' ) . ' <strong>' . esc_html__( 'Date', 'godecore' ) . '</strong></a></li>';
	$html	 .= '<li class="' . (($pob == 'popularity') ? 'current' : '') . '"><a href="' . esc_url( vedanta_addURLParameter( $query_string, 'product_orderby', 'popularity' ) ) . '">' . esc_html__( 'Sort by', 'godecore' ) . ' <strong>' . esc_html__( 'Popularity', 'godecore' ) . '</strong></a></li>';
	$html	 .= '<li class="' . (($pob == 'rating') ? 'current' : '') . '"><a href="' . esc_url( vedanta_addURLParameter( $query_string, 'product_orderby', 'rating' ) ) . '">' . esc_html__( 'Sort by', 'godecore' ) . ' <strong>' . esc_html__( 'Rating', 'godecore' ) . '</strong></a></li>';
	$html	 .= '</ul>';
	$html	 .= '</li>';
	$html	 .= '</ul>';
...
function vedanta_woocommerce_get_catalog_ordering_args( $args ) {
	global $woocommerce;
	...
	if ( $pob == 'rating' ) {
		...
		add_filter( 'posts_clauses', 'vedanta_order_by_rating_post_clauses' );
	}

	return $args;
}

function vedanta_order_by_rating_post_clauses( $args ) {
	global $wpdb;
...
function vedanta_loop_shop_per_page() {
...
function vedanta_add_bootstrap_input_classes( $args, $key, $value = null ) {
...
add_filter( 'woocommerce_form_field_args', 'vedanta_add_bootstrap_input_classes', 10, 3 );
...
add_action( 'woocommerce_view_order', 'vedanta_woocommerce_view_order', 10 );

remove_action( 'woocommerce_thankyou', 'woocommerce_order_details_table', 10 );
add_action( 'woocommerce_thankyou', 'vedanta_woocommerce_view_order', 10 );

function vedanta_woocommerce_view_order( $order_id ) {
	global $woocommerce;
	...
	?>
	<div class="vedanta-order-details woocommerce-content-box">
		<div class="sec-head-style"><h3 class="text-title"><?php esc_html_e( 'Order Details', 'godecore' ); ?></h3></div>
		<div class="table-responsive">
			<table class="table cart-table order_details">
				<thead>
					<tr>
						<th class="col-title"></th>
						<th class="col-title"><?php esc_html_e( 'Product', 'godecore' ); ?></th>
						<th class="col-quantity"><?php esc_html_e( 'Quantity', 'godecore' ); ?></th>
						<th class="col-subtotal"><?php esc_html_e( 'Total', 'godecore' ); ?></th>
					</tr>
				</thead>
				<tfoot>
					<?php
...
function instance_search_result() {
	if ( apply_filters( 'vedanta_check_ajax_referer', true ) ) {
		check_ajax_referer( '_vedanta_nonce', 'nonce' );
	}
	$response = array();
	...
	if ( empty( $response ) ) {
		$response[] = sprintf( '<li>%s</li>', esc_html__( 'Nothing found', 'godecore' ) );
	}
...
add_action( 'wp_ajax_vedanta_search_products', 'instance_search_result' );
add_action( 'wp_ajax_nopriv_vedanta_search_products', 'instance_search_result' );
 ...
 * 
 */
function vedanta_woocommerce_taxonomy_archive() {
...
add_action( 'woocommerce_archive_description', 'vedanta_woocommerce_taxonomy_archive', 10 );
 ...
 * @global string $post
 */
function vedanta_product_share() {
...
add_action( 'wp_ajax_vedanta_product_quick_view', 'product_quick_view' );
add_action( 'wp_ajax_nopriv_vedanta_product_quick_view', 'product_quick_view' );
...
// QuicKview
add_action( 'vedanta_single_product_summary', 'get_product_quick_view_header', 5 );
...
add_action( 'yith_wcqv_product_summary', 'vedanta_product_share', 35 );

/**
 * Elementor Elements Product Summary
 */
add_action( 'vedanta_product_link_open', 'woocommerce_template_loop_product_link_open' );
add_action( 'vedanta_product_thumbnail', 'vedanta_template_loop_product_thumbnail' );
add_action( 'vedanta_product_link_close', 'woocommerce_template_loop_product_link_close' );
add_action( 'vedanta_product_title', 'vedanta_template_loop_product_title' );
add_action( 'vedanta_product_rating', 'vedanta_wc_shop_loop_item_rating' );
add_action( 'vedanta_product_price', 'woocommerce_template_loop_price' );
add_action( 'vedanta_product_description', 'vedanta_template_loop_product_description' );
add_action( 'vedanta_product_add_to_cart', 'woocommerce_template_loop_add_to_cart' );
add_action( 'vedanta_product_item', 'vedanta_template_loop_product_item' );
add_action( 'vedanta_product_countdown_timer', 'vedanta_template_loop_product_countdown_timer' );
add_action( 'vedanta_product_progress_bar', 'vedanta_template_loop_product_progress_bar' );

if ( !function_exists( 'vedanta_template_loop_product_thumbnail' ) ) {

	/**
	 * Get the product thumbnail for the loop.
	 * 
	 */
	function vedanta_template_loop_product_thumbnail() {
		$thumbnail = woocommerce_get_product_thumbnail();
		echo apply_filters( 'vedanta_template_loop_product_thumbnail', $thumbnail );
	}

}

if ( !function_exists( 'vedanta_template_loop_product_title' ) ) {
	 ...
	 * 
	 */
	function vedanta_template_loop_product_title() {
		global $product;
...
if ( !function_exists( 'vedanta_template_loop_product_description' ) ) {

	/**
	 * Get the product short-description for the loop.
	 */
	function vedanta_template_loop_product_description( $product ) {
	?>
...
if ( !function_exists( 'vedanta_template_loop_product_item' ) ) {

	/**
	 * Get the product buttons for the loop.
	 */
	function vedanta_template_loop_product_item($product) {
		?>
		<div class="woo-product-btn">
			<?php
				do_action( 'vedanta_product_add_to_cart'  );
				vedanta_product_actions_add_wishlist_link();
				vedanta_product_actions_add_quick_view_link();
			?>
		</div>
		<?php
	}
...
if ( !function_exists( 'vedanta_template_loop_product_countdown_timer' ) ) {

	/**
	 * Get the sale product countdown timer for the loop.
	 * 
	 */
	function vedanta_template_loop_product_countdown_timer( $product ) {
		...
		if ( $time_diff > 0 ) :
			...
			<div class="deal-countdown-timer">
				<span class="deal-time-diff" style="display:none;"><?php echo apply_filters( 'vedanta_deal_countdown_timer_diff_time', $time_diff ); ?></span>
...
if ( !function_exists( 'vedanta_template_loop_product_progress_bar' ) ) {

	/**
	 * Get the sale product progress bar for the loop.
	 * 
	 */
	function vedanta_template_loop_product_progress_bar() {
		...
		if ( $stock_available > 0 ) :
			...
			<div class="deal-progress">
				<div class="deal-stock">
					<span class="stock-sold"><?php echo esc_html__( 'Already Sold:', 'godecore' ); ?> <strong><?php echo esc_html( $stock_sold ); ?></strong></span>
					<span class="stock-available"><?php echo esc_html__( 'Available:', 'godecore' ); ?> <strong><?php echo esc_html( $stock_available ); ?></strong></span>
				</div>
				<div class="progress">
