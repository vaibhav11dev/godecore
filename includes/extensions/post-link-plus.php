 ...
 */
function vedanta_get_adjacent_post_plus($r, $previous = true) {
    global $post, $wpdb;
 ...
 */
function vedanta_previous_post_link_plus($args = '') {
    return vedanta_adjacent_post_link_plus($args, '&laquo; %link', true);
}
 ...
 */
function vedanta_next_post_link_plus($args = '') {
    return vedanta_adjacent_post_link_plus($args, '%link &raquo;', false);
}
 ...
 */
function vedanta_adjacent_post_link_plus($args = '', $format = '%link &raquo;', $previous = true) {
    $defaults = array(
...
//	If Post Types Order plugin is installed, default to sorting on menu_order
    ...
    if ($previous && is_attachment()) {
        ...
        $posts = vedanta_get_adjacent_post_plus($r, $previous);
...
//	When num_results > 1, multiple adjacent posts may be returned. Use foreach to display each adjacent post.
    foreach ($posts as $post) {
        ...
        if (empty($post->post_title))
            $title = $previous ? esc_html__('Previous Post', 'godecore') : esc_html__('Next Post', 'godecore');
...
//		Truncate the link title to nearest whole word under the length specified.
        ...
        $anchor = '<a href="' . esc_url(get_permalink($post)) . '" rel="' . $rel . '"' . $tooltip . '>';
...
//		Optionally add the post thumbnail to the link. Wrap the link in a span to aid CSS styling.
        if ($r['thumb'] && has_post_thumbnail($post->ID)) {
            ...
            $thumbnail = '<a class="post-thumbnail" href="' . esc_url(get_permalink($post)) . '" rel="' . $rel . '"' . $tooltip . '>' . get_the_post_thumbnail($post->ID, $r['thumb']) . '</a>';
