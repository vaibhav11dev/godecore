...
$ved_pagination_type = vedanta_get_option( 'ved_pagination_type', 'pagination' );
...
<?php } else { ?>
	<div class="navigation-links page-navigation clearfix">
	    ...
	    if ( $ved_pagination_type == "number_pagination" ) {
			...
			<?php echo vedanta_paginate_links(); ?>
		    </nav>
	    <?php } else { ?>
			<div class="col-xs-6 col-md-6 nav-next"><?php previous_posts_link( sprintf('<i class="fa fa-angle-left p-r-10"></i> %s', esc_html__( 'Newer Entries', 'godecore' ) ) ); ?></div>
		    <div class="col-xs-6 col-md-6 nav-previous"><?php next_posts_link( sprintf('%s <i class="fa fa-angle-right p-l-10"></i>', esc_html__( 'Older Entries', 'godecore' ) ) ); ?></div>
	    <?php } ?>
	</div>
	<div class="clearfix"></div> 
	<?php
